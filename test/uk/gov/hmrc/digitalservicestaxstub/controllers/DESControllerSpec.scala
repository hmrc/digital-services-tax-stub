...
class DESControllerSpec extends AnyFreeSpec with GuiceOneServerPerSuite with Matchers {
  ...
  def controller = new DESController(appConfig = appConfig, cc = cc, authAndEnvAction)
  ...
  "TaxEnrolmentCallbackController" - {
    ...
    "dstRegistration" - {
      s"must return OK for DST regime" in {
        val result: Future[Result] = controller.dstRegistration("DST", "utr", "1111111000")(registrationRequest)
        status(result) mustBe OK
